...
- [x] **Real target/client registry (`addTarget`/`addClient`).** This is the
      ...
      referenced in this codebase's own type names). Do this _before_ adding a
...
- [x] **A couple more language clients** once the registry + 2 clients have
      ...
      **Effort: M each. Priority: later.** — _Fixed:
      `src/targets/rust/tokio-tungstenite/client.ts` (async/await on Tokio,
      `serde_json::json!` for payloads, `IntoClientRequest` for handshake
      headers) and `src/targets/go/gorilla/client.ts` (`websocket.DefaultDialer`,
      `map[string]interface{}` payload literal, `http.Header` for handshake
...
- [ ] **Add a "why AsyncAPI 3.x, and what does resolution actually look
      ...
      A second example fixture showing an _unresolved_ placeholder in the output
