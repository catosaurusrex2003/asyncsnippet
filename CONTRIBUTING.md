...
Built-in targets/clients (`javascript/ws`, `javascript/websocket`, `javascript/kafkajs`, `python/websockets`, `python/confluent-kafka`, `rust/tokio-tungstenite`, `rust/rdkafka`, `go/gorilla`, `go/kafka-go`, `shell/websocat`, `agent/ws`, `agent/kafka`) are registered this same way at the bottom of `src/targets/index.ts` — that's the reference example to copy for a new one. Both functions are also exported from the package root (`import { addTarget, addTargetClient } from "asyncsnippet"`) for consumers who want to register their own target/client without forking the library.
